...
async fn run_client(
    ...
    // TODO(jared): make request ListBlockDevices
            ...
            loop {
                tokio::select! {
                    ...
                    Some(Ok(msg)) = stream.next() => {
                        match msg {
                            Response::NewDevice(dev) => {
                                write!(stdout, "{}{}", termion::cursor::Goto(1, print_offset), dev.name)?;
                                print_offset += 1;
                                ...
                                for entry in dev.boot_entries {
                                    ...
                                    write!(stdout, "{}{}", termion::cursor::Goto(3, print_offset), entry.display)?;
                                    print_offset += 1;
                                }
