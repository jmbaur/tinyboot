{ config, pkgs, lib, ... }: {
  config = lib.mkIf (config.board == "trogdor-wormdingler") {
    ...
    linux = {
      configFile = with pkgs.tinybootKernelConfigs; lib.mkDefault (pkgs.concatText "trogdor-wormdingler-kernel.config" [ generic video aarch64 chromebook qcom sc7180 ./kernel.config ]);
