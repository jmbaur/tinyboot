{ config, pkgs, lib, kconfig, ... }: {
  config = lib.mkIf (config.board == "trogdor-wormdingler") {
    ...
    linux = {
      configFile = with pkgs.tinybootKernelConfigs; lib.mkDefault (pkgs.concatText "trogdor-wormdingler-kernel.config" [ generic aarch64 chromebook qcom ./kernel.config ]);
