...
pub const LOG_BUFFER_SIZE = 2 << 11;
pub var log_buffer: ?[]align(std.mem.page_size) u8 = null;
pub var console_comm_fds = [_]?os.fd_t{null} ** 10; // arbitrary limit of 10 consoles
var offset: usize = 0;

pub fn initLogger() !void {
    log_buffer = try os.mmap(null, LOG_BUFFER_SIZE, os.PROT.READ | os.PROT.WRITE, os.MAP.SHARED | os.MAP.ANONYMOUS, -1, 0);
}
...
pub fn logFn(
    ...
    var buf = [_]u8{0} ** 256;
    const print = std.fmt.bufPrint(&buf, level_txt ++ prefix2 ++ format ++ "\n", args) catch return;
