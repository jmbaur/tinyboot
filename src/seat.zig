...
pub const Seat = struct {
    ...
    pub fn register(self: *@This(), epoll_fd: posix.fd_t) !void {
        ...
        for (self.consoles) |console| {
            ...
            try posix.epoll_ctl(epoll_fd, os.linux.EPOLL.CTL_ADD, console.comm_fd, &event);
        }
    }
    ...
    pub fn force_shell(self: *@This()) void {
        for (self.consoles) |console| {
            ...
            _ = posix.write(console.comm_fd, std.mem.asBytes(&msg)) catch return;
        }
    }
    ...
    pub fn handle_new_event(self: *@This(), event: os.linux.epoll_event) !?posix.RebootCommand {
        for (self.consoles) |console| {
            if (console.comm_fd == event.data.fd) {
                ...
                _ = try posix.read(console.comm_fd, std.mem.asBytes(&msg));
    ...
    pub fn deinit(self: *@This()) void {
        for (self.consoles) |console| {
            std.posix.close(console.comm_fd);

            _ = os.linux.pidfd_send_signal(console.pid_fd, posix.SIG.USR1, null, 0);
            _ = posix.waitpid(console.pid, 0);
            posix.close(console.pid_fd);
        }
    }
};
...
test "smoke test" {
    ...
    var seat = try Seat.init(&[_]posix.fd_t{ console1[0], console2[0] }, .{
...

    {
        ...
        const n_written = try posix.write(console1[1], buf_input);
        var buf = try std.testing.allocator.alloc(u8, n_written);
        defer std.testing.allocator.free(buf);
        const n_read = try posix.read(seat.stdin_fd, buf);
        ...
        const n_echo = try posix.read(console2[1], echo_buf);
...

    {
        ...
        const n_written = try posix.write(console2[1], buf_input);
        var buf = try std.testing.allocator.alloc(u8, n_written);
        defer std.testing.allocator.free(buf);
        const n_read = try posix.read(seat.stdin_fd, buf);
        try std.testing.expectEqual(n_written, n_read);
        ...
        var echo_buf = try std.testing.allocator.alloc(u8, n_written);
        defer std.testing.allocator.free(echo_buf);
        const n_echo = try posix.read(console1[1], echo_buf);
        try std.testing.expectEqualSlices(u8, buf_input[0..n_written], echo_buf[0..n_echo]);
    }
}
