...
// stupid that we have to do this, but this determines the inner type of std.time.Instant.
...
test "generalized time" {
    ...
    try std.testing.expectEqualStrings("19700100000000Z", try generalizedTime(.{
        .timestamp = if (is_posix) .{ .sec = 0, .nsec = 0 } else 0,
    }, &buf));

    try std.testing.expectEqualStrings("20250512050449Z", try generalizedTime(.{
        .timestamp = if (is_posix) .{ .sec = 1747112689, .nsec = 0 } else 1747112689,
    }, &buf));
}
