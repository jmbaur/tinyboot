...
pub fn match(device: *const Device) bool {
    ...
    const sysfs_disk_path = device.nodeSysfsPath(&sysfs_disk_path_buf) catch return false;

    var sysfs_dir = std.fs.cwd().openDir(sysfs_disk_path, .{}) catch return false;
    defer sysfs_dir.close();

    // disks have a "removable" file in their sysfs directory
    _ = sysfs_dir.access("removable", .{}) catch return false;
...
pub fn probe(
    ...
    const partition_path = try boot_partition.nodePathZ(&partition_path_buf);

    var partition = try std.fs.cwd().openFileZ(partition_path, .{});
...
fn mount(self: *DiskBootLoader, fstype: FsType, path: [*:0]const u8) !void {
    // make sure there are no current mountpoints
    try self.unmount();

    const tmpdir = try TmpDir.create(.{});

    try tmpdir.dir.makePath("mount");
